                ...
                if (args.length == 1) {
                    Message.send(
                            "There are currently "
                                    ...
                                    + (DropManager.instance.size() == 1 ? " stat" : " stats")
                                    + " available.");
                    Message.send(
                            "Do /"
                                    + getName()
                                    + " trace list to see all of them in brief context.");
                    Message.send(
                            "Or do /"
                                    + getName()
                                    + " trace <index> to see the specific of that stat.");
                    Message.send(
                            "Do /"
                                    + getName()
                                    + " trace delete <index> to delete that specific index.");
                    Message.send(
                            "Additionally you can do /"
                                    + getName()
                                    + " trace delete all to wipe them.");
                    return;
                }
                int idx;
                int kills = 0;
                ...
                switch (args[1].toLowerCase(Locale.ROOT)) {
                    case "list":
                        ...
                        for (int i = 0; i < DropManager.instance.size(); i++) {
                            ...
                            final double inAvg =
                                    (tmp.getQuantity(DropStatistics.INGREDIENT) <= 0
                                            ? 0
                                            : (double) tmp.getKills() / ing);
                            Message.send(
                                    "Cache #"
                                            + (i + 1)
                                            + ": §r"
                                            + tmp.getKills()
                                            + "§a kills; §r"
                                            + tmp.getQuantity(DropStatistics.ALL)
                                            + "§a drops §7(§r"
                                            + item
                                            + "§7 items, §r"
                                            + ing
                        ...
                        Message.send(
                                "Stats (Avg.): Kills: "
                                        + kills
                                        + " ("
                                        + df.format(kills / divisor)
                                        + "), Items: "
                                        + items
                                        + " ("
                                        + df.format(items / divisor)
                                        + "), Ingredients: "
                                        + ingredients
                                        + " ("
                                        + df.format(ingredients / divisor)
                                        + ")");
                        return;
                    case "delete":
                        if (args.length == 2) {
                            Message.send(
                                    "Invalid input. Try /" + getName() + " trace for more info.");
                            return;
                        }
            ...
            case "note":
                if (args.length == 1) {
                    Message.send(
                            "/"
                                    + getName()
                                    + " note all [note] - Sets all drop data with the note.");
                    Message.send(
                            "/"
                                    + getName()
                                    + " note last [note] - Sets last drop data with the note.");
                    Message.send(
                            "/"
                                    + getName()
                                    + " note # [note] - Sets specified drop data with this note.");
                ...
                switch (args[1].toLowerCase(Locale.ROOT)) {
                    case "all":
                        ...
                        for (Iterator<DropStatistics> it = DropManager.instance.iterator();
                                it.hasNext(); ) {
            ...
            case "export":
                ...
                if (args.length == 1) {
                    Message.send(
                            "/"
                                    + getName()
                                    + " export all [name]: Exports all session data as JSON.");
                    Message.send(
                            "/"
                                    + getName()
                                    + " export range [name] <min> [max]: Exports session data in range as JSON.");
                    Message.send(
                            "/"
                                    + getName()
                                    + " export last [name]: Exports last session data as JSON.");
                    Message.send(
                            "/" + getName() + " export # [name]: Exports the #th index as JSON.");
                    Message.send(
                            "View stats via /"
                                    + getName()
                                    + " trace. There are "
                                    + ENTRY_SIZE
                                    + (ENTRY_SIZE == 1 ? " entry" : " entries")
                                    + " available.");
                    return;
                } else {
                    ...
                    switch (args[1].toLowerCase(Locale.ROOT)) {
                        case "all":
                            DropManager.exportDrops(
                                    fileName,
                                    new ArrayList<>(DropManager.instance.getBackingList()));
                            break;
                        case "last":
                            DropManager.exportDrops(
                                    fileName,
                                    Collections.singletonList(DropManager.instance.getLast()));
                            break;
                        case "range":
                            if (args.length == 3) {
                                ...
                                try {
                                    ...
                                    Message.send(
                                            "Index out of bounds. Max allowed size: " + ENTRY_SIZE);
                                    return;
                                }
                            } else
                                try {
                        ...
                        default:
                            try {
                                ...
                                DropManager.exportDrops(
                                        fileName,
                                        Collections.singletonList(DropManager.instance.get(x)));
                            } catch (NumberFormatException ex) {
                                Message.send("Invalid range specified.");
                                return;
                            } catch (IndexOutOfBoundsException e) {
