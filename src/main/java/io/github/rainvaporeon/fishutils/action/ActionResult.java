 ...
 */
/**
 * An object representing the execution status of an action. This is bundled with a {@link Result}
 * denoting the execution result, and can hold a return value and a throwable if an exception had
 * occurred.
 *
 * <p>The implementation also supports multiple chaining statements to provide ease of execution,
 * such as {@link ActionResult#expect(Class, Consumer)} to handle specific exceptions, throwing
 * exceptions with {@link ActionResult#throwIfPresent()} and such.
 *
 * @param <T> The return type
 * @apiNote This object is not meant to be serialized, and only recommended to be used as return
 *     values for simplifying handling the result of executing something.
 * @since 1.0
 * @author Rain
 */
public final class ActionResult<T> implements Action<T> {
    ...
    /**
     * Creates a new ActionResult holding the result and a return value; the throwable is null in
     * this case
     *
    ...
    /**
     * Creates a new ActionResult holding the result and a throwable; the return value is null in
     * this case
     *
     * @param result The result
     ...
     * @apiNote if execution fails, the returned ActionResult still will hold this object's return
     *     value.
     */
     ...
     * @apiNote if one of the exceptions were handled successfully, then this is considered a
     *     "handled result", and so calling {@link ActionResult#getReturnValue()} will not throw the
     *     exception.
     */
    ...
    /**
     * Retrieves the underlying return value. May raise exceptions if an unhandled one is present.
     *
     * @return the return value
     * @apiNote this method will throw an exception if one is present and is not handled.
     ...
     * @apiNote if this method is called, it's assumed that the exception is handled, and therefore
     *     any future calls of {@link ActionResult#getReturnValue()} will not throw an exception
     *     unless any processing methods recreated one. It's not like you'd usually retrieve a
     *     return value and an exception, but this can come in handy if you are handling an
     *     exception safely and need a default return value anyway.
     */
    ...
    /**
     * Utility method to handle any throwable if the result is not {@link Result#SUCCESS}. Useful
    ...
    /** Convenience method to create an ActionResult that holds {@code null} as the return type. */
    public static <T> ActionResult<T> success() {
        return (ActionResult<T>) DEFAULT_SUCCESS;
    }

    /**
     * Convenience method to create an ActionResult that the provided element as the return type.
     */
    ...
    /**
     * Convenience method to create an ActionResult with the current stacktrace as the exception.
     */
    ...
    /**
     * Convenience method to create an ActionResult with the provided throwable as the exception.
     *
     * @param throwable The throwable, nullable.
     * @return An ActionResult holding {@link Result#FAIL}, or {@link Result#ERROR} if throwable is
     *     not null
     */
     ...
     * @return a success action with the value if no exceptions occurred, otherwise return a failed
     ...
     * @return an action holding the result and value if no exceptions occurred, otherwise return a
     *     failed action with the exception
     */
     ...
     * @return a success action if no exceptions occurred, otherwise return a failed action with the
     *     exception
     */
    ...
    /**
     * Throws the exception this object currently holds if it is present, otherwise return itself
     *
     * @return itself
     */
    ...
    public boolean equals(Object o) {
        ...
        return exceptionHandled == that.exceptionHandled
                && result == that.result
