 ...
 *
 * <p>The bus provides several utility methods to listen to certain events and to fire them, only
 * the bus subscribers may find them.
 *
 * <p>All methods that are to receive events from the event bus should be annotated with {@link
 * io.github.rainvaporeon.fishutils.utils.eventbus.events.EventBusSubscriber} in order for the bus
 * to acknowledge the class and fire appropriate event.
 */
public class EventBus {
    ...
    /**
     * Whether the bus should be able to receive the same event multiple times. If this is {@code
     * false}, this bus will not write a signature to the event.
     */
     ...
     *
     * @param processDuplicates whether this bus should be able to receive the same event multiple
     *     times
     * @apiNote "Same event" in this context does not indicate that for event a and b, if {@code
     *     a.equals(b)} returns true, the event is discarded, but rather if {@code a == b} is true,
     *     the event is discarded. In other words, the event is discarded if and only if {@code
     *     processDuplicates} is set to false (default) and if the two events have identical
     *     identity.
     */
    ...
    /**
     * Hooks this event bus to a certain parent, listening to all events from the said parent.
     *
     * <p>This method does a deep recursion check to see whether the parent inherits from this bus,
     * if so, an {@link IllegalStateException} will be thrown.
     *
     * @param parent the parent
     * @apiNote This will add the current bus instance to the parent's inheritance set, in which is
     *     called every time the parent fires an event. In other words, this bus will receive every
     *     event the parent receives. In some structures, the event may be received multiple times.
     *     If this is intended, the event bus should be created with {@link
     *     EventBus#EventBus(boolean)}} and with "processMulti" set to true.
     */
    ...
    protected static void check(Object o) {
        ...
        // throw new InvalidSubscriberException("class " + o.getClass() + " does not have any
        // subscriber methods");
    }
